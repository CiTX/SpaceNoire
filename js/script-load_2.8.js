...
$.getScript("https://dl.dropbox.com/s/2oa8bn6d30sjjuk/radio.js");
...
function iTPM(str,e){$(".pm-input."+e).val($(".pm-input."+e).val()+str).focus()}
(function(){let j=setTimeout(function k(){if($('[id^="pm-"].pm-panel').length==0){}if($('[id^="pm-"].pm-panel').length>0){if($('[id^="pm-"].pm-panel').length==$('#pmline').length){}else{(function(){let a=document.querySelectorAll('[id^="pm-"].pm-panel');for(let b of a){let a=function(){let c=$(b).attr("id").split("pm-")[1];if($("#pm-"+c+".pm-panel .panel-body").children("input.form-control.pm-input").hasClass(c)){}else{$("#pm-"+c+".pm-panel .panel-body").children("input.form-control.pm-input").addClass(c);$("#pm-"+c+".pm-panel .panel-body").children("input.form-control.pm-input").attr("id","pmline");$("#pm-"+c+".pm-panel .panel-body").children("input.form-control.pm-input").after('<span id="'+c+'-smiles-btn" class="pm-smiles-btn btn btn-sm btn-default">'+'<img src="https://i.imgur.com/DT3sXCp.png" style="width: 18px;height: 18px;">'+'</span>');window["pmeps_" +c]=0;$("#pm-"+c+".pm-panel .panel-body .pm-buffer.linewrap").append('<div class="row" id="'+c+'-emotesrow" style="position: absolute;z-index: 5;left: 0% !important;max-height: calc(100% - 80px) !important;max-width: 100%;background-position: center;overflow-y: auto;padding: 0px;top: 40px;"><div class="col-lg-12 col-md-12" style="display: none;margin: 0px;"><div class="well" id="'+c+'-emoteslist" style="text-align: center;background-color: rgba(0,0,0,0.7);padding: 0px;margin-top:-20px;margin-bottom:-20px;margin-left: 0px;margin-right: -15px;"></div></div></div>');let smiles=CHANNEL.emotes;function pmrs(e){if(window["pmeps_"+c]!=0){$("#"+e+"-emotesrow > div").hide();$('#'+e+'-smiles-btn').removeClass('btn-success').addClass('btn-default');window["pmeps_" +c]=0}else{$("#"+e+"-emotesrow > div").show();$('#'+e+'-smiles-btn').removeClass('btn-default').addClass('btn-success');window["pmeps_" +c]=1;}}for(var smileIndex = 0, smilesLen = smiles.length; smileIndex < smilesLen; smileIndex++){$('<img class="pmemote">').attr({src: smiles[smileIndex].image, onclick: 'iTPM(" ' + smiles[smileIndex].name + ' ","'+c+'")'}).data('name', smiles[smileIndex].name).appendTo("#"+c+"-emoteslist")}$('#'+c+'-smiles-btn').on('click',function(){pmrs(c)});}};a()}})()}}j=setTimeout(k,200);},20);$(`<style>`).appendTo("head").text(`.panelemote{max-height: 30px;}.pm-input{width:calc(100% - 32px);margin-right:32px;}.pm-smiles-btn{position: relative;z-index: 0;top: -37px;left: calc(100% - 32px);height:37px;width: 32px;padding-left: 3px;}.form-control.pm-input{z-index: 10 !important;padding: 0px 0px;}.pm-smiles-btn > img{width: 24px !important;height: 24px !important;vertical-align: text-top;}.pmemote{max-height: 40px;max-width: 40px;width:auto;height: 30px;margin:3px;}.pm-panel > .panel-body {margin-bottom: -37px}.pm-input {padding-left: 5px !important;padding-right: 5px !important;}`);})();
$("#videowrap-btns").remove(); function changeVideoWidth(direction){var body=document.body;if(/hd/.test(body.className)){throw new Error("ui::changeVideoWidth does not work with the 'hd' layout")}var videoWrap=document.getElementById("videowrap");var leftControls=document.getElementById("leftcontrols");var leftPane=document.getElementById("leftpane");var chatWrap=document.getElementById("chatwrap");var rightControls=document.getElementById("rightcontrols"); var rightPane=document.getElementById("rightpane");var match=videoWrap.className.match(/col-md-(\d+)/);if(!match){throw new Error("ui::changeVideoWidth: videowrap is missing bootstrap class!")}var videoWidth=parseInt(match[1],10)+direction;if(videoWidth<3||videoWidth>9){return}var chatWidth=12-videoWidth;videoWrap.className="col-md-"+videoWidth+" col-lg-"+videoWidth;rightControls.className="col-md-"+videoWidth+" col-lg-"+videoWidth;rightPane.className="col-md-"+videoWidth+" col-lg-"+videoWidth;chatWrap.className="col-md-"+chatWidth+" col-lg-"+chatWidth;leftControls.className="col-md-"+chatWidth+" col-lg-"+chatWidth;leftPane.className="col-md-"+chatWidth+" col-lg-"+chatWidth;handleVideoResize()}$("#currenttitle").before(`<p id="videowrap-btns"></p>`);$("#videowrap-btns").append(`<span class="glyphicon glyphicon-plus pointer" id="resize-video-larger" title="Делает окно видео больше"></span>`);$("#videowrap-btns").append(`<span class="glyphicon glyphicon-minus pointer" id="resize-video-smaller" title="Делает окно видео меньше"></span>`);$("#resize-video-larger").click(function(){try{changeVideoWidth(1)}catch(error){console.error(error)}});$("#resize-video-smaller").click(function(){try{changeVideoWidth(-1)}catch(error){console.error(error)}});
